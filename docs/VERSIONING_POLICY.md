...
Together these backward-compatible refinements define the **v2.2 minor workflow revision**. They add material verification and novelty-discrimination obligations inside existing Stages and therefore are not a patch. They do not add, remove, renumber, merge, or reroute any canonical Stage and therefore are not a major release. The published `v2.1` tag remains immutable until a separate `v2.2` tag/release is actually created.
